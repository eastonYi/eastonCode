...
from tfModels.coldFusion import cold_fusion, simple_cold_fusion
...
class AC_LM_Decoder(object):
    ...
    def __init__(self, args, is_train, global_step, embed_table=None, name=None):
        # use decoder2 heres
        self.num_layers = args.model.classifier.num_layers
        self.num_hidden_size = args.model.classifier.num_hidden_size
        self.dropout = args.model.classifier.dropout
        self.dim_output = args.dim_output
        dim_ctc_output = args.dim_ctc_output if args.dim_ctc_output else args.dim_output
        dim_encoder_output = args.model.encoder.bottleneck
        frame_expand = args.model.frame_expand * (3 if args.model.use_neighbor_frames else 1)
        self.ac_vector = frame_expand * dim_encoder_output \
        ...
        def step(i, preds, logits, cache_lm):
            ...
            if self.args.model.acoustic_corrupt:
                ...
                frame = tf.cond(sample,
                        lambda: encoded[:, i, :] + tf.random_normal(tf.shape(encoded[:, i, :]), stddev=50.0),
                        lambda: encoded[:, i, :])
            else:
                frame = encoded[:, i, :]
            ...
            with tf.variable_scope(self.args.top_scope, reuse=True):
                with tf.variable_scope(self.args.lm_scope):
                    ...
                    lm_output = dense(
                        ...
                        use_bias=False)
                    # z_lm = tf.nn.log_softmax(logit_lm) # [batch*beam, size_output]

                    lm_output = tf.stop_gradient(lm_output)
                    cache = tf.stop_gradient(cache)

            # lm_vector = lm_output[:, -1, :]
            lm_vector = lm_output

            # generate the feature for the classifier
            # feature = tf.concat([frame, lm_vector], axis=-1)
            feature = frame

            with tf.variable_scope('classifier'):
                for j in range(self.args.model.classifier.num_layers):
                    feature = tf.layers.dense(
                        inputs=feature,
                        units=self.num_hidden_size,
            ...
            self.params = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='RNA_Model/classifier')
            ...
            return i+1, preds, logits, cache_lm

        _, preds, logits, _ = tf.while_loop(
            cond=lambda i, *_: tf.less(i, tf.shape(encoded)[1]),
            body=step,
            loop_vars=[0, token_init, logits_init, cache_lm_init],
