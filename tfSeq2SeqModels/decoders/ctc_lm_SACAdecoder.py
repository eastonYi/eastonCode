...
class CTC_LM_SACA_Decoder(RNADecoder):
    ...
    def __init__(self, args, is_train, global_step, embed_table=None, name=None):
        ...
        self.frame_expand = (3 if args.model.use_neighbor_frames else 1) * args.model.frame_expand
        self.num_cell_units_en = dim_encoder_output \
    ...
    def _decode(self, encoded, len_encoded):
        ...
        def step(i, preds, cache_decoder, logits):
            encoder_output_i = tf.reshape(encoded[:, i, :], [batch_size, self.frame_expand, self.num_cell_units_en])
    ...
    def beam_decode_rerank(self, encoded, len_encoded):
        ...
        def step(i, preds, scores, cache_decoder, cache_lm, logits):
            ...
            """
            encoder_output_i = tf.reshape(encoded[:, i, :], [batch_size * beam_size, self.frame_expand, self.num_cell_units_en])
            encoder_attention_bias_i = tf.equal(tf.reduce_sum(tf.abs(encoder_output_i), axis=-1), 0.0)
            encoder_attention_bias_i = common_attention.attention_bias_ignore_padding(encoder_attention_bias_i)
        ...
        _, preds, scores_am, _, _, logits = tf.while_loop(
            cond=lambda i, *_: tf.less(i, tf.shape(encoded)[1]),
            body=step,
            loop_vars=[0, token_init, scores, cache_decoder_init, cache_init, logits_init],
            shape_invariants=[tf.TensorShape([]),
                              tf.TensorShape([None, None]),
                              tf.TensorShape([None]),
                              tf.TensorShape([None, None, None, None]),
