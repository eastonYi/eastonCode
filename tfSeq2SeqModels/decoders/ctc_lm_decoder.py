...
class CTC_LM_Decoder(RNADecoder):
    ...
    def _decode(self, encoded, len_encoded):
        ...
        if self.args.model.cold_fusion:
            self.num_cell_units_cold_fusion = self.args.model.decoder2.num_cell_units_cold_fusion
            # cache for self-attention lm model
            all_initial_states["state_lm"] = tf.zeros([
                batch_size,
                0,
                self.lm.args.model.decoder.num_blocks,
                self.lm.args.model.decoder.num_cell_units])
            shape_state_lm = tf.zeros([
                batch_size,
                batch_size, # only use the batch_size to create a None shape
                self.lm.args.model.decoder.num_blocks,
                self.lm.args.model.decoder.num_cell_units]).shape
            shape_loop_states['state_lm'] = shape_state_lm
            ...
            if self.args.model.cold_fusion:
                with tf.variable_scope(self.args.top_scope, reuse=True):
                    with tf.variable_scope(self.args.lm_scope):
                        logit_lm, state_lm = self.lm.decoder.forward(
                            preds,
                            all_states["state_lm"],
                            stop_gradient=True)
                ...
                if True:
                    cur_logit = cold_fusion(
                        ...
                        num_cell_units=self.num_cell_units_cold_fusion,
