...
class Agent(object):
    ...
    def process_raw_input(self, frames, len_frames):
        size_batch  = tf.shape(frames)[0]
        size_length = tf.shape(frames)[1]
        size_feat = int(self.size_feat/3)
        x = tf.reshape(frames, [size_batch, size_length, size_feat, 3])
        ...
        len_frames = tf.cast(tf.ceil(tf.cast(len_frames,tf.float32)/8), tf.int32)
